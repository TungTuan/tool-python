...
async def create_video(request: VideoRequest):
    try:
        image_files_dict = [img.dict() for img in request.image_files]
        print({"image_files": image_files_dict})
        # Call the video creation function
        create_video_with_custom_durations(
            image_files_dict, 
            request.audio_file, 
            request.subtitles_file, 
            request.background_video_file, 
            request.output_file
        )
        print(f"Error 111: { request.image_files}")
        return {"message": "Video created successfully", "output_file": request.output_file}
...
def create_video_with_custom_durations(image_files, audio_file, subtitles_file, background_video_file, output_file):
    ...
    try:
        ...
        total_image_duration = sum(max(0, img['duration'] - 0.5) for img in image_files)
        ...
        remaining_duration = audio_duration - sum(max(0, img['duration'] - 0.5) for img in images_to_loop)
